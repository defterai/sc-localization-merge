...
class LocalizationIni:
    ...
    @staticmethod
    def FromXlsxFiles(filenames, columnsCount):
        result = []
        if columnsCount > 0:
            dataArray = []
            columnIndex = 0
            while columnIndex < columnsCount:
                dataArray.append(collections.OrderedDict())
                columnIndex += 1
        ...
        with codecs.open(inputFilename, "r", "utf-8") as inputFile:
            while True:
                ...
                if not prefix:
                    break;
                if prefix != '\\u':
                    raise Exception("Found invalid prefix: {0}".format(prefix))
                codepoint = inputFile.read(4)
                if not codepoint:
                    raise Exception("Missing codepoint")
                characterSet.add(chr(int(codepoint, 16)))
    except Exception as err:
        raise Exception("Parse codepoints file error: {0}".format(err))
    return characterSet
